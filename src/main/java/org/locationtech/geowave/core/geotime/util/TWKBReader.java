...
public class TWKBReader {
 ...
 private Polygon readPolygon(
   ...
   final LinearRing exteriorRing =
       fac.createLinearRing(precision.readPointArray(input));
   final LinearRing[] interiorRings = new LinearRing[numRings - 1];
   for (int i = 0; i < (numRings - 1); i++) {
     interiorRings[i] =
         fac.createLinearRing(precision.readPointArray(input));
   }
   return fac.createPolygon(exteriorRing, interiorRings);
 }
 ...
 private MultiPoint readMultiPoint(
   ...
   return fac.createMultiPointFromCoords(points);
 }
 ...
 private MultiPolygon readMultiPolygon(
   ...
   for (int i = 0; i < numPolygons; i++) {
     ...
     polygons[i] = fac.createPolygon(exteriorRing, interiorRings);
   }

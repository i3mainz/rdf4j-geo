...
public class Overlaps extends RasterVectorRelationBinaryFunction {
	...
	public Boolean relation(Value v1, Value v2) {
		...
		if(type==null || type2==null) {
			...
			return vec1.read(v1.stringValue()).overlaps(LiteralUtils.toGeometry(vec2.read(v2.stringValue()).getEnvelope()));
		}else if(!type && type2) {
			RasterLiteral vec1=(RasterLiteral) LiteralRegistry.getLiteral(lit1.getDatatype().toString());
			VectorLiteral vec2=(VectorLiteral) LiteralRegistry.getLiteral(lit2.getDatatype().toString());
			return LiteralUtils.toGeometry(((GridCoverage2D)vec1.read(v1.stringValue())).getEnvelope()).overlaps(vec2.read(v2.stringValue()));
		}else {
			RasterLiteral vec1=(RasterLiteral) LiteralRegistry.getLiteral(lit1.getDatatype().toString());
			RasterLiteral vec2=(RasterLiteral) LiteralRegistry.getLiteral(lit2.getDatatype().toString());
			return LiteralUtils.toGeometry(((GridCoverage2D)vec1.read(v1.stringValue())).getEnvelope()).overlaps(LiteralUtils.toGeometry(vec2.read(v2.stringValue()).getEnvelope()));
		}
	}

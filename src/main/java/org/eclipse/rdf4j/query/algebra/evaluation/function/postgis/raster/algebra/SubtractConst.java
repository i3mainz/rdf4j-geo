...
public class SubtractConst extends RasterAlgebraConstFunction {

	@Override
	public String getURI() {
		return POSTGIS.ST_rast_algebra_subtractconst.stringValue();
	}
	...
	public GridCoverage modify(GridCoverage raster,Integer rd1,Double constt) {
		...
		RenderedOp subtractedImage = JAI.create("subtractconst", pbSubtracted);
		final SampleDimension sd = new SampleDimension.Builder().setName("t")
				.addQuantitative(
						(raster.getSampleDimensions().get(rd1).getName() + " subtractconst "
								+ constt).toString(),
